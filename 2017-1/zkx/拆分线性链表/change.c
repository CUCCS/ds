...
void CreateList(LinkList L, int n)  //创建原线性链表
{
	int i;
	LNode* p = L;
	L->next = NULL;  
...
{
	LNode*p;     //用于遍历list,list1,list2的三个指向节点的指针
...
/*用尾指针rear来表示单循环链表，
则查找开始结点a1和终端结点an都很方便，
它们的存储位置分别是(rear->next)->next和rear，
显然，查找时间都是O(1)。故所用时间最少，空间最少*/