...
status InitHashTable(HashTable *H)
{
	...
	{
		(*H).elem[i].key = -1; // 未填的标注为-1
		(*H).elem[i].val = 0;
	}
	hashsize_count= hashsize_count+1;
	return 1;
}
//进行插入操作
...
{
	int i;
	int count = (*H).sizeindex;//H中原有记录个数  
	ElemType *p;
	ElemType *elem = (ElemType *)malloc(count * sizeof(ElemType));//动态生成存放哈希表H原有数据的空间 
	p = elem;
	for (i = 0; i < (*H).sizeindex; ++i)
	{
		//将原有的所有记录，保存到elem中  	  
	...
	hashsize_count= hashsize_count+1;//增加表长下标 为下一次重建做准备
	return 1;
}
int main()
{
